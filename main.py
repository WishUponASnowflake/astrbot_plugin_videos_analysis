...
@register("hybird_videos_analysis", "喵喵", "可以解析抖音和bili视频", "0.1.5","https://github.com/miaoxutao123/astrbot_plugin_videos_analysis")
...
async def auto_parse_bili(self, event: AstrMessageEvent, context: Context, *args, **kwargs):
    ...
    message_str = event.message_str
    try:
        message_data = json.loads(event.message_str)
    except json.JSONDecodeError:
        print("消息解析失败，message_str 不是有效的 JSON 格式")
        
    if message_data :
        # 提取 qqdocurl
        try:
            qqdocurl = message_data["message"][0]["data"]["meta"]["detail_1"]["qqdocurl"]
